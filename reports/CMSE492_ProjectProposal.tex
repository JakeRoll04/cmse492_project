...
Table~\ref{tab:metric-comparison} summarizes the validation performance for the three models.
...
(Table~\ref{tab:time-comparison}). Logistic regression trains almost instantly, making it suitable
...
To cross-check these findings in a simpler linear setting, RFE was applied to a logistic regression model trained on the same one-hot encoded features. The RFE results, shown in Figure~\ref{fig:rfe-logreg}, identify a very similar subset of important predictors, including education level, capital gain, marital status, and hours-per-week. This agreement between the tree-based Gradient Boosting model and the linear logistic regression model increases confidence that these features are genuinely important rather than artifacts of a specific model class.
...
Three supervised classification models---Logistic Regression, Random Forest, and Gradient Boosting---
were trained and compared using accuracy, precision, recall, and F1 score on the validation dataset.
Gradient Boosting emerged as the strongest model, achieving the highest accuracy (0.871) and the
